...
export default function MetroModel(props) {
  const { nodes, scene } = useGLTF("/metro.gltf");
  ...
  useFrame(({ viewport }) => {
    ...
    scene.position.x = xOffset * 0.003 - 3.5;
    scene.position.y = yOffset * 0.2;
    scene.lookAt(x - 3, y - 18.5, 8);
  });

  useFrame(({ clock }) => {
    scene.position.y = Math.sin(clock.getElapsedTime()) * 0.08 - 36;
